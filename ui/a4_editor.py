...
class A4GraphicsView(QGraphicsView):
    ...
    def contextMenuEvent(self, event):
        ...
        if action == act_del:
            ...
            item.set_item_scale(item.item_scale() * 1.25, snap=True)
        elif action == act_zout:
            item.set_item_scale(item.item_scale() / 1.25)
...
class CardProcessingThread(QThread):
    ...
    def run(self):
        try:
            ...
            with ThreadPoolExecutor(max_workers=workers) as pool:
                ...
                for fut in as_completed(futs):
                    ...
                    try:
                        ...
                        logger.error("فشل معالجة الصورة %d/%d: %s", idx + 1, n, e)
                        data = self._SENTINEL
                    results[idx] = data
            ...
            logger.critical("خطأ غير متوقع في خيط المعالجة", exc_info=True)
        finally:
...
class A4Editor(QWidget):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("محرر بطاقات الهوية - A4")
        self.resize(900, 700)
        self.setLayoutDirection(Qt.RightToLeft)
        self.cards = []
        self._num_pages = 1
    ...
    def _build_ui(self):
        ...
        btn_zoom_in.clicked.connect(lambda: self._zoom_selected(1.2))
        btn_zoom_out = QPushButton("🔍-")
        btn_zoom_out.setToolTip("تصغير")
        btn_zoom_out.clicked.connect(lambda: self._zoom_selected(1 / 1.2))
    ...
    def _apply_default_zoom(self):
        settings = QSettings("ورشة طباعة", "A4Editor")
    ...
    def _copy_selected(self):
        ...
        for item in items:
            if isinstance(item, IDCardItem):
                ...
                logger.info("تم نسخ البطاقة (تدوير %.0f°)", self._copied_rotation)
                return
    ...
    def _rotate_selected(self):
        ...
        for item in items:
            if isinstance(item, IDCardItem):
                item.set_item_rotation((item.item_rotation() + 90) % 360)
                logger.info("تم تدوير البطاقة إلى %.0f°", item.item_rotation())
        ...
        for i, c in enumerate(self.cards):
            if i // MAX_CARDS == last_page:
                QMessageBox.warning(self, "تنبيه",
                    "لا يمكن حذف الصفحة الأخيرة لأنها تحتوي على بطاقات.")
                return
    ...
    def _on_drop_batch(self, paths):
        ...
        self._spinner.show()
        logger.info("بدأت معالجة %d صورة في الخلفية", len(paths))

    def add_image_dialog(self):
        paths, _ = QFileDialog.getOpenFileNames(
            self, "اختر صور الهوية", "",
    ...
    def _on_processing_done(self):
        self._spinner.hide()
        logger.info("انتهت معالجة جميع الصور")
    ...
    def add_image(self, path_or_bytes):
        ...
        try:
            ...
            if not qpix.loadFromData(buf.getvalue()):
                raise ValueError("فشل تحميل الصورة المعالجة")
            self._place_card(qpix)
        except Exception as e:
            logger.error("فشل إضافة الصورة", exc_info=True)
    ...
    def _place_card(self, pixmap: QPixmap):
        ...
        logger.info("تمت إضافة البطاقة %d (صفحة %d)", len(self.cards), idx // MAX_CARDS + 1)
    ...
    def _swap_cards(self, a, b):
        ...
        logger.info("تم تبديل البطاقتين %d و %d", i + 1, j + 1)
    ...
    def clear_all(self):
        ...
        self.scene.setSceneRect(0, 0, A4_W, A4_H)
        self._draw_page_area(0)
        logger.info("تم تفريغ جميع البطاقات")
    ...
    def save_pdf(self):
        ...
        path, _ = QFileDialog.getSaveFileName(self, "حفظ كـ PDF", "", "ملفات PDF (*.pdf)")
        if not path:
            return
        if getattr(self, 'subscription_check', lambda: True)():
            selected = self.scene.selectedItems()
            ...
            if not painter.isActive():
                logger.error("فشل بدء الرسم لحفظ PDF")
                QMessageBox.warning(self, "خطأ", "فشل في حفظ PDF")
                return
            try:
